...
function getFiltered(tab){{
  ...
  if(tab==='open'){{
    ...
    const bc=parseInt(r.BUY_COUNT||0);
    const cfgDef=CONFIGS_DEF.find(c=>c.id===r.CONFIG||c.ID===r.CONFIG)||{{}};
    const maxBuys=parseInt(cfgDef.max_buys||cfgDef.MAX_BUYS||4);
    if(bc>=maxBuys) return false;
    const nextBuy=avg*(1-dropPct);
    // Show only when LTP is within 2% ABOVE next buy trigger (be-ready zone)
    return avg>0&&ltp>0&&ltp>nextBuy&&ltp<=nextBuy*1.02;
  }}).sort((a,b)=>{{
    const ca=cfgMap[a.CONFIG]||0.1, cb_=cfgMap[b.CONFIG]||0.1;
    const na=(parseFloat(a.AVG_BUY_PRICE)||0)*(1-ca);
    const nb=(parseFloat(b.AVG_BUY_PRICE)||0)*(1-cb_);
    const da=na>0?((parseFloat(a.CURRENT_LTP)||0)-na)/na*100:999;
    const db=nb>0?((parseFloat(b.CURRENT_LTP)||0)-nb)/nb*100:999;
    return da-db;
  }});

  renderBadges('avgtrigger-badges', rows, 'open');
  const body=document.getElementById('avgtrigger-body');
  document.getElementById('avgtrigger-info').textContent=`${{rows.length}} stocks within 2% of avg buy trigger`;
  
  if(!rows.length){{body.innerHTML='<tr><td colspan="10" class="empty">No stocks within 2% of avg buy trigger right now</td></tr>';return;}}
